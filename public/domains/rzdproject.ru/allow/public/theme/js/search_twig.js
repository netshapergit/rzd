document.addEventListener('DOMContentLoaded', function() {
	...
	const catFilters = {
		projects: {
			types: [
				{name: 'Внедрение', value: 'Внедрение'},
				{name: 'Адаптация решения для ОАО РЖД', value: 'Адаптация решения для ОАО РЖД'},
				{name: 'Заявка на финансирование', value: 'Заявка на финансирование'},
				{name: 'Подготовка ТТ (ТЗ)', value: 'Подготовка ТТ (ТЗ)'},
				{name: 'Рассмотрение ФЗ результатов испытаний', value: 'Рассмотрение ФЗ результатов испытаний'},
				{name: 'Подготовка ТЭО по результатам испытаний', value: 'Подготовка ТЭО по результатам испытаний'},
				{name: 'Испытания', value: 'Испытания'},
				{name: 'Формирование ПМИ', value: 'Формирование ПМИ'}
			],
			products: [
				{name: 'Технология', value: 'Технология'},
				{name: 'Оборудование', value: 'Оборудование'},
				{name: 'Промышленная продукция', value: 'Промышленная продукция'},
				{name: 'Сервисные и иные услуги', value: 'Сервисные и иные услуги'},
				{name: 'Другое', value: 'Другое'}
			],
			readiness: [
				{name: 'Незавершенная стадия ОКР', value: 'Незавершенная стадия ОКР'},
				{name: 'Завершенные опытно-конструкторские и экспериментальные исследования', value: 'Завершенные опытно-конструкторские и экспериментальные исследования'},
				{name: 'Испытания опытного образца', value: 'Испытания опытного образца'},
				{name: 'Совершенствование отдельных элементов инновационного  продукта  приналичии проработанной технологии производства', value: 'Совершенствование отдельных элементов инновационного  продукта  приналичии проработанной технологии производства'},
				{name: 'Подготовлено производство', value: 'Подготовлено производство'},
				{name: 'Ведется серийное производство', value: 'Ведется серийное производство'},
				{name: 'Другое', value: 'Другое'}
			]
		},
		requests: {
			status: [
				{name: 'Плановые', value: 'Плановые'},
				{name: 'Архив', value: 'Архив'}
			],
			zakaz: [
				{name: 'ДИ', value: 'ДИ'},
				{name: 'ДМС', value: 'ДМС'},
				{name: 'ДКРЭ', value: 'ДКРЭ'},
				{name: 'ТЭ', value: 'ТЭ'},
				{name: 'РЦДМ', value: 'РЦДМ'},
				{name: 'ДМВ', value: 'ДМВ'},
				{name: 'ДРП', value: 'ДРП'},
				{name: 'НС', value: 'НС'},
				{name: 'Т', value: 'Т'},
				...
				{name: 'ЦШ', value: 'ЦШ'},
				{name: 'КЦМ', value: 'КЦМ'},
				{name: 'ЦТЕХ', value: 'ЦТЕХ'},
				{name: 'ДПО', value: 'ДПО'},
				{name: 'ДЭЗ', value: 'ДЭЗ'},
				{name: 'ДМ', value: 'ДМ'},
				{name: 'СИВЦ', value: 'СИВЦ'},
				{name: 'ЦДИ ЦП', value: 'ЦДИ ЦП'},
				{name: 'КДЗ', value: 'КДЗ'},
				{name: 'ДСС', value: 'ДСС'},
				{name: 'КХТЛ', value: 'КХТЛ'},
				{name: 'П', value: 'П'},
				{name: 'НТЭ', value: 'НТЭ'}
			],
			year: [
	...
	const filtersVoc = {
		'Текущий статус': 'type',
		'Результат проекта': 'product',
		'Степень готовности проекта': 'readiness',
		'Статус': 'status',
		'Заказчик': 'zakaz',
		'Год': 'year'
	}
	...
	window.App = new Vue({
		...
		data: function() {
			return {
				...
				selectedCatFilters: {
					...
					projects: {
						type: {type: 'or', items: []},
						product: {type: 'or', items: []},
						readiness: {type: 'or', items: []}
					},
					requests: {
						status: {type: 'or', items: []},
						zakaz: {type: 'or', items: []},
						year: {type: 'or', items: []}
					}
				},
		...
		methods: {
			resetFilters(filterTypeName) {
				for(let key in this.selectedCatFilters[filterTypeName]) {
					this.selectedCatFilters[filterTypeName][key].items = []
				}
			},
			setPageItems() {
				...
				*/
				...
				switch(LID) {
					...
					case '35': 
						...
						break;
					case '48':
						nested = nestedFilter(this.items, this.selectedCatFilters.projects)
						break;
					case '50':
						nested = nestedFilter(this.items, this.selectedCatFilters.requests)
						break;
					default:
		...
		mounted() {
			...
			jQuery('.filter__title').on('click', function() {
				jQuery(this).siblings('.filter-main').slideToggle(200)
			})
